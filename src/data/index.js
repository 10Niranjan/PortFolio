// ============================================
// PERSONAL DATA — Niranjan Patil
// ============================================

export const personal = {
  name: "Niranjan Patil",
  shortName: "Niranjan",
  initials: "NP",
  role: "Mobile Developer & Flutter Engineer",
  tagline: "Designing high-fidelity interfaces and modern web solutions.",
  bio: "I'm a mobile developer & software engineer based in Pune, India. I specialize in building cross-platform mobile applications and intelligent web systems using Flutter, Dart, and Java. Passionate about clean code, elegant UI, and solving complex engineering problems.",
  location: "Pune, India 🇮🇳",
  availability: "Open to Work",
  email: "pvt.niranjan10@gmail.com",
  phone: "+91 9284774471",
  resumeUrl: "/resume.pdf",
  socials: {
    github: "https://github.com/10Niranjan",
  },
};

export const stats = [
  { value: "132",  label: "Liters of coffee consumed" },
  { value: "50K+", label: "Total lines of code written" },
  { value: "100+", label: "DSA problems solved on LeetCode" },
  { value: "3",    label: "Major projects delivered" },
  { value: "7.0",  label: "CGPA in BE Information Technology" },
];

export const highlights = [
  "Building IndShield — AI-powered industrial safety with YOLOv8 & OpenCV.",
  "Fanclub Cricket App — cross-platform Flutter development with real-time data.",
  "Java To-Do App — CRUD task management with JavaFX & console interface.",
  "Solving 100+ Java DSA problems on LeetCode — still counting.",
  "Google Cybersecurity Foundations — certified.",
];

export const projects = [
  {
    id: 1,
    title: "Svarae",
    description: "Premium eCommerce store with custom glassmorphism, dynamic routing, state-managed cart drawer, and single-page checkout flow.",
    tags: ["Next.js", "Tailwind CSS", "Zustand", "Prisma", "CockroachDB", "Cloudinary"],
    link: "https://svarae-app.vercel.app",
    github: "https://github.com/10Niranjan",
    size: "large",
    inDevelopment: true,
  },
  {
    id: 2,
    title: "Campus Connect",
    description: "Interactive campus collaboration platform featuring student forums, event boards, lost-and-found listings, and peer-to-peer marketplace.",
    tags: ["React", "Node.js", "Express", "MongoDB", "Socket.io", "Tailwind CSS"],
    link: "https://github.com/10Niranjan",
    github: "https://github.com/10Niranjan",
    size: "large",
    inDevelopment: true,
  },
  {
    id: 3,
    title: "Fanclub Cricket",
    description: "Cross-platform Flutter app for cricket fans — live scores, match schedules, player stats, fan polls, and real-time chat rooms.",
    tags: ["Flutter", "Dart", "Android Studio"],
    link: "#",
    github: "https://github.com/10Niranjan",
    size: "large",
  },
  {
    id: 4,
    title: "To-Do List App",
    description: "Intuitive task manager with full CRUD operations, due dates, and status tracking — available as both console and JavaFX GUI.",
    tags: ["Java", "JavaFX", "MySQL"],
    link: "#",
    github: "https://github.com/10Niranjan",
    size: "small",
  },
  {
    id: 5,
    title: "React 19 Architecture",
    description: "High-performance web architecture leveraging React 19 concurrent features and fluid UI animations.",
    tags: ["React 19", "Vite", "Framer Motion"],
    link: "#",
    github: "https://github.com/10Niranjan",
    size: "small",
  },
  {
    id: 6,
    title: "Next.js Fullstack",
    description: "Scalable full-stack application featuring Next.js App Router, dynamic routing, and advanced caching.",
    tags: ["Next.js", "React", "TypeScript"],
    link: "#",
    github: "https://github.com/10Niranjan",
    size: "small",
  },
  {
    id: 7,
    title: "GitHub",
    description: "See more projects & open source contributions.",
    tags: [],
    link: "https://github.com/10Niranjan",
    github: "https://github.com/10Niranjan",
    size: "small",
    isGithub: true,
  },
];


export const skills = [
  {
    label1: "Mobile",
    label2: "development",
    items: ["Flutter", "Dart", "Android Studio", "Flutter SDK"],
    color: "#7c3aed",
  },
  {
    label1: "Java",
    label2: "engineering",
    items: ["Core Java", "JavaFX", "Gradle", "OOPs", "Data Structures"],
    color: "#f59e0b",
  },
  {
    label1: "Web &",
    label2: "backend",
    items: ["HTML", "CSS", "Flask", "Jinja2", "Python"],
    color: "#10b981",
  },
  {
    label1: "AI & Computer",
    label2: "vision",
    items: ["YOLOv8", "OpenCV", "TensorFlow", "MediaPipe", "Scikit-learn"],
    color: "#ec4899",
  },
  {
    label1: "Database &",
    label2: "tools",
    items: ["MySQL", "SQL", "Power BI", "GitHub", "VS Code"],
    color: "#3b82f6",
  },
];
